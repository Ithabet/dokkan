...
@section('EXTCSS')
    ...
    <link href="{{ URL::to('assets/plugins/select2/css/select2-bootstrap.min.css') }}" rel="stylesheet"
          type="text/css"/>
    <link href="{{ URL::to('assets/plugins/material-datetimepicker/bootstrap-material-datetimepicker.css') }}"
          rel="stylesheet"/>
...
@section('content')
                        ...
                        <div class="row">
                            <div class="col-sm-4">
                                <div class="row">
                                    ...
                                    <div class="col-sm-12">
                                        <select id="CustomerID" name="customer_id" onchange="get_customer_data(this)"
                            ...
                            <div class="col-sm-4">
                                <div class="row">
                                    ...
                                    <div class="col-sm-12">
                                        <input id="receiptDate" class="mdl-textfield__input" type="text"
                                               value="{{ date('Y-m-d') }}" id="date" data-dtp="dtp_JFOV1">
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <select id="orderType" class="form-control  select2">
                                    <option value="0" {{($sale->order_type==0) ? 'selected' : ''}}>صالة</option>
                                    {{--                                    <option value="1">تيك اواي</option>--}}
                                    <option value="2" {{($sale->order_type==2) ? 'selected' : ''}}>ديليفري</option>
                                </select>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-4" hidden>
                                        ...
                                        <div class="input-group spinner">
                                            ...
                                            <input type="number" id="selected_product_quantity"
                    ...
                    <div class="card-body ">
                        ...
                        <div id="productsList">
                            @foreach($sale->items as $item)
                                <div data-id="{{$item->product_id}}" class="row product product_{{$item->product_id}}">
                                    <div class="col-sm-4">
                                        <input type="hidden" value="{{$item->product_id}}" class="ProductID"
                                               name="productID[]"><label class="ProductName"
                                                                         data-id="{{$item->product_id}}"
                                                                         data-name="{{$item->product_id}}">{{$item->product->name}}</label>
                                    </div>
                                    <div class="col-sm-2">
                                        <input type="number" data-id="{{$item->product_id}}" name="productQuantity[]"
                                               class="form-control input-sm text-center quantity "
                                               value="{{$item->product_quantity}}" min="1">
                                    </div>
                                    <div class="col-sm-2"><input name="productPrice[]" type="text"
                                                                 data-id="{{$item->product_id}}" value="{{$item->product_price}}"
                                                                 class="form-control text-center input-sm price "></div>
                                    <div class="col-sm-2"><span class="total total_{{$item->product_id}}">{{$item->product_price * $item->product_quantity}} </span>
                                        <input type="hidden" value="{{$item->product_id}}" class="ProductTotal"
                                               name="productTotal[]"></div>
                                    <div class="col-sm-2">
                                        <button data-id="{{$item->product_id}}" type="button"
                                                class="btn btn-sm btn-danger btn-block remove"><span
                        ...
                        <div id="totals" style="display: none">
                            ...
                            <div class="row">
                                <div class="col">
                                    <button type="button" id="ConfirmSale" data-toggle="modal"
                                            data-target="#ConfirmSalePopUp"
                                            class="btn-confirm-sale btn btn-lg btn-block btn-success">تنفيذ
                                    </button>
                                </div>
                            </div>
                        </div>
        ...
        <div class="col-md-5 col-sm-5">
            <div class="card card-box">
                <div class="card-head">
                    ...
                    <div class="btn-group pull-left">
                        <input type="text" id="SearchByName" placeholder="بحث بإسم المنتج" dir="rtl"
                               class="form-control">
                    </div>
                </div>
                <div class="card-body " id="bar-parent">
    ...
    <div class="modal fade" id="ConfirmSalePopUp" tabindex="-1" role="dialog" aria-labelledby="ConfirmSalePopUpTitle"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ URL::to('sales/update/'.$sale->id) }}" method="post">
                    {{ csrf_field() }}
                    ...
                    <div class="modal-body">
                        <div id="final-recipt">
                            ...
                            <table class="table">
                                ...
                                <tr>
                                    <td>رقم الهاتف</td>
                                    <td><input type="text" id="sale_phone" name="sale_phone" value=""
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <input id="f_receipt_total" autofocus type="text" readonly name="total"
                                       class="form-control">
                            </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-sm-4">
                                <input id="f_receipt_paid" value="0" autofocus type="text" name="paid"
                                       class="form-control">
                            </div>
                            <div class="col-sm-2">
...
@section('EXTJS')
    ...
    <script>
        $(function () {
            ...
            var productsList = [];
            @foreach($sale->items as $item)
                productsList.push({{$item->product_id}});
            @endforeach
